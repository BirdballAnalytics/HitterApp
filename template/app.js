...
function zoneScatter(div, rows, {colorField, colorMap, title, tooltipFn}){
  ...
  const layout = {
    ...
    shapes: zoneShapes(),
    legend:{orientation:'h', y:-0.22, font:{size:9}, itemwidth:30},
    margin:{l:45,r:10,t:34,b:46}, height:320
  };
...
function zoneDensity(div, rows, title, height=300, dark=false){
  ...
  const topMargin = hasTitle ? (height<250?16:24) : Math.min(6, Math.round(height*0.08));
  const layout = {
    title: hasTitle ? {text:`${title}${dark?'':` (n=${rows.length})`}`, font:{size:height<250?9:11, color: dark?'#93a0b0':undefined}} : undefined,
    ...
    margin:{l:4,r:4,t:topMargin,b:4}, height, showlegend:false,
...
function xyScatter(div, rows, {xField, yField, xRange, yRange, colorField, colorMap, title, xLabel, yLabel, tooltipFn, height=360}){
  ...
  const legendRows = Math.ceil(cats.length / 6);
  const layout = {
    title:{text:title, font:{size:12}},
    ...
    legend:{orientation:'h', y:-0.05 - 0.09*legendRows, font:{size:9}, itemwidth:30},
    margin:{l:44,r:8,t:30,b:38 + 16*legendRows}, height: height + 16*legendRows
  };
  Plotly.newPlot(div, traces, layout, {displayModeBar:false, responsive:true});
}
...
function renderVeloOverTime(rows){
  ...
  panel.innerHTML = `<div class="chart-box"><div id="velo-time" style="height:460px;"></div></div>`;
  const sorted = rows.slice().sort((a,b)=>(a.Date||'').localeCompare(b.Date));
  ...
  cats.forEach(cat=>{
    ...
    traces.push({ type:'scatter', mode:'markers', name:PITCH_ABBR[cat]||cat, x:xs, y:ys,
  ...
  const legendRows = Math.ceil(cats.length / 7);
  Plotly.newPlot(document.getElementById('velo-time'), traces, {
    ...
    legend:{orientation:'h', y:-0.08-0.09*legendRows, font:{size:9}, itemwidth:30},
    margin:{l:50,r:10,t:34,b:60+16*legendRows}
  }, {displayModeBar:false, responsive:true});
}
...
function renderStrikeOverTime(rows){
  ...
  panel.innerHTML = `<div class="chart-box"><div id="strike-time" style="height:460px;"></div></div>`;
  const sorted = rows.slice().sort((a,b)=>(a.Date||'').localeCompare(b.Date));
  const cats = [...new Set(sorted.map(r=>r.Pitch))].sort();
  const traces = [];
  cats.forEach(cat=>{
