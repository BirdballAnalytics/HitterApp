...
function zoneDensity(div, rows, title, height=300){
  ...
  const layout = {
    title:{text:`${title} (n=${rows.length})`, font:{size:height<250?9:11}},
    ...
    shapes: zoneShapes(),
    margin:{l:6,r:6,t:24,b:6}, height, showlegend:false,
    plot_bgcolor:'#fff'
  };
  Plotly.newPlot(div, traces, layout, {displayModeBar:false, responsive:true});
}
...
function renderScoutReport(rows){
  ...
  panel.innerHTML = `
    ...
    <div class="scout-top-grid">
      <div class="scout-tables-col">
        <div class="section-label" style="margin-top:0;">Traditional Stats</div>
        <div id="scout-summary"></div>
        <div id="scout-bypitch" style="margin-top:4px;"></div>
        <div class="section-label" style="margin-top:10px;">Metrics</div>
        <div id="scout-metrics"></div>
      </div>
      <div class="scout-plot-col">
        <div class="section-label" style="margin-top:0;">Pitch Plot</div>
        <div class="chart-box"><div id="scout-pitchplot"></div></div>
      </div>
    </div>

    <div class="section-label chart-row">Right-Handed Hitters — Locations &amp; Damage</div>
    ...
    <div class="section-label chart-row">Left-Handed Hitters — Locations &amp; Damage</div>
    <div class="grid grid-4 scout-heat-grid">
  ...
  xyScatter(document.getElementById('scout-pitchplot'), rows, {
    ...
    xLabel:'Horizontal Break', yLabel:'Induced Vertical Break', tooltipFn:tipM, height:270
  });
...
window.addEventListener('beforeprint', ()=>{
  ...
  document.querySelectorAll('#panel-scoutreport [id^="scout-"]').forEach(div=>{
    if (div.dataset && window.Plotly && div._fullLayout) window.Plotly.Plots.resize(div);
  });
});
