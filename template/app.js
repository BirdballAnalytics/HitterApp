...
/* Simple 2x2 strike-zone quadrant chart, catcher's-eye view, no axis labels.
   The quadrant with the highest "damage" rate (hits or 95+ EV, among pitches
   located in that quadrant) is highlighted green — read as "this is where a
   hitter should expect to do damage against this pitcher." No interpretation
   reference image was provided, so this is a best-guess design; happy to
   restyle once you can share the exact chart you have in mind. */
function renderDamageQuadrant(div, rows, sideLabel){
  ...
  const cell = key => `<div class="dmg-cell${key===best?' dmg-hi':''}"></div>`;
  div.innerHTML = `
    <div class="dmg-card">
      <div class="dmg-title">${sideLabel}</div>
      <div class="dmg-grid">
...
function renderScoutReport(rows){
  ...
  panel.innerHTML = `
    ...
    <div class="scout-top-grid-3">
      ...
      <div>
        <div class="section-label" style="margin-top:0;">Damage Zone</div>
        <div class="scout-dmg-row">
          <div id="scout-dmg-lhh-zone"></div>
          <div id="scout-dmg-rhh-zone"></div>
        </div>
      </div>
    </div>

    <div class="section-label chart-row">TrackMan Data</div>
  ...
  xyScatter(document.getElementById('scout-pitchplot'), rows, {
    ...
    xLabel:'Horizontal Break', yLabel:'Induced Vertical Break', tooltipFn:tipM, height:230
  });

  renderDamageQuadrant(document.getElementById('scout-dmg-lhh-zone'), rows.filter(r=>r.BatterSide==='Left'), 'LHH');
  renderDamageQuadrant(document.getElementById('scout-dmg-rhh-zone'), rows.filter(r=>r.BatterSide==='Right'), 'RHH');
