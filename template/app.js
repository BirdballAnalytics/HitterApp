...
/* ============================================================
   ...
   ============================================================ */
const state = {
  mode: null,        // 'hitting' | 'pitching' | 'scout'
  season: "2026 SPRING",
  dateStart: null,
  dateEnd: null,
  player: "All",
...
function renderMiniSpray(div, rows, title){
  ...
  Plotly.newPlot(div, [
    ...
    { type:'scatter', mode:'markers', x:pts.map(p=>p.x), y:pts.map(p=>p.y),
      marker:{ size:4, color: pts.map(p=>p.damage?'#fb923c':'#5b6472'), opacity:0.85, line:{width:0} },
    ...
    margin:{l:2,r:2,t:16,b:2}, height:120, plot_bgcolor:'#10141a', paper_bgcolor:'#10141a', showlegend:false
...
function battericonSVG(flip){
  const t = flip ? ' style="transform:scaleX(-1)"' : '';
  return `<svg viewBox="0 0 24 48" width="100%" height="100%"${t}>
    <circle cx="12" cy="6" r="4" fill="#5b6472"/>
    <path d="M12 10 L12 26 M12 14 L4 20 M12 14 L20 10 M12 26 L6 44 M12 26 L18 44"
      stroke="#5b6472" stroke-width="2.5" fill="none" stroke-linecap="round"/>
  </svg>`;
}
...
function cheatSheetRow(name, team, rows){
  ...
  const heatTile = (side) => `
    ...
    <div class="cheat-battericon">${battericonSVG(side==='l')}</div>`;
  ...
  return { name, html, draw: ()=>{
    renderMiniSpray(document.getElementById(`${uid}_spray_r`), rhh, '');
    renderMiniSpray(document.getElementById(`${uid}_spray_l`), lhh, '');
    zoneDensity(document.getElementById(`${uid}_dmg_r`), rhh.filter(r=>r.InPlay&&(r.H===1||r.HardHit===1)), '', 60, true);
    zoneDensity(document.getElementById(`${uid}_dmg_l`), lhh.filter(r=>r.InPlay&&(r.H===1||r.HardHit===1)), '', 60, true);
    zoneDensity(document.getElementById(`${uid}_bbos_r`), rhh.filter(r=>r.OffSpeed), '', 60, true);
    zoneDensity(document.getElementById(`${uid}_bbos_l`), lhh.filter(r=>r.OffSpeed), '', 60, true);
    zoneDensity(document.getElementById(`${uid}_fb_r`), rhh.filter(r=>r.FBc), '', 60, true);
    zoneDensity(document.getElementById(`${uid}_fb_l`), lhh.filter(r=>r.FBc), '', 60, true);
