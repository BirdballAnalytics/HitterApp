...
function filteredPlayers(){
  ...
  if (state.mode === 'pitching'){
    ...
    ALL_ROWS.forEach(r=>{
      if (r.PitcherTeam && r.PitcherTeam!==TEAM && inRange(r.Date,s,e)) set.add(`${r.Pitcher}||${r.PitcherTeam}`);
    });
  } else {
    ALL_ROWS.forEach(r=>{
...
function scoutMetricsRow(rows){
  return {
    Pitches: rows.length,
    "Avg Velo": fmt1(mean(rows,r=>r.RelSpeed)), "Max Velo": fmt1(maxOf(rows,r=>r.RelSpeed)),
    "Avg Spin": fmt0(mean(rows,r=>r.SpinRate)), "Max Spin": fmt0(maxOf(rows,r=>r.SpinRate)),
    IVB: fmt1(mean(rows,r=>r.IVB)), HB: fmt1(mean(rows,r=>r.HB)),
    "Rel Height": fmt1(mean(rows,r=>r.RelHeight)), Extension: fmt1(mean(rows,r=>r.Extension)),
    VAA: fmt1(mean(rows,r=>r.VAA)), HAA: fmt1(mean(rows,r=>r.HAA)),
  };
}

/* ── Scout Tab: one-page opposing pitcher scouting report ──
   "Damage" is defined here as any batted ball that was either a hit or
   hit hard (95+ EV) — i.e. locations where this pitcher has gotten burned. */
function renderScoutReport(rows){
  const panel = document.getElementById('panel-scoutreport');
  if (!state.player){
    panel.innerHTML = `<div class="empty-msg">No opposing pitchers found in this date range.</div>`;
    return;
  }
  if (!rows.length){
  ...
  panel.innerHTML = `
    <div class="section-label" style="font-size:13px;color:#333;border:none;margin-top:0;">
      ${pName} <span style="color:#999;font-weight:400;">— ${pTeam} — Throws ${throws} — ${rows.length} pitches across ${games} game(s)</span>
    </div>

    <div class="section-label">Traditional Stats</div>
    <div id="scout-summary"></div>
    <div id="scout-bypitch" style="margin-top:6px;"></div>

    <div class="section-label chart-row">Pitch Plot</div>
    <div class="chart-box" style="max-width:640px;"><div id="scout-pitchplot"></div></div>

    <div class="section-label chart-row">Metrics</div>
    <div id="scout-metrics"></div>

    <div class="section-label chart-row">Locations — Right-Handed Hitters</div>
    <div class="grid grid-3">
    ...
    <div class="section-label chart-row">Locations — Left-Handed Hitters</div>
    <div class="grid grid-3">
    ...
    <div class="section-label chart-row">Damage Allowed</div>
    <div class="grid grid-3">
      <div class="chart-box"><div id="scout-dmg-rhh"></div></div>
      <div class="chart-box"><div id="scout-dmg-lhh"></div></div>
    </div>`;

  renderTable(document.getElementById('scout-summary'), pitchingSummary(rows), {rateCols:['BAA','SLG','WHIP','FIP']});
  const byPitch = groupBy(rows, r=>r.Pitch);
  renderTable(document.getElementById('scout-bypitch'),
    sortPitchKeys([...byPitch.keys()]).map(p=>({Pitch:p, ...pitchingByPitchStats(byPitch.get(p))})),
    {pctCols:['IZ %','Swing%','Z-Take%','Whiff%','Chase%','Contact%','Barrel%','GB%','FB%','LD%']});
  ...
  xyScatter(document.getElementById('scout-pitchplot'), rows, {
    xField:'HB', yField:'IVB', xRange:[-30,30], yRange:[-30,30],
    colorField:'Pitch', colorMap:PITCH_COLORS, title:'Pitch Shapes',
    xLabel:'Horizontal Break', yLabel:'Induced Vertical Break', tooltipFn:tipM
  });

  renderTable(document.getElementById('scout-metrics'),
    sortPitchKeys([...byPitch.keys()]).map(p=>({Pitch:p, ...scoutMetricsRow(byPitch.get(p))})));

  const isCH = r => r.Pitch === 'ChangeUp' || r.Pitch === 'Changeup';
  const draw = (id, side, filterFn, title) => zoneDensity(document.getElementById(id),
    rows.filter(r=>r.BatterSide===side && filterFn(r)), title);
  draw('scout-rhh-fb', 'Right', r=>r.FBc, 'RHH — Fastballs');
  draw('scout-rhh-bb', 'Right', r=>r.BBc, 'RHH — Breaking Balls');
  draw('scout-rhh-ch', 'Right', isCH,     'RHH — Changeups');
  draw('scout-lhh-fb', 'Left',  r=>r.FBc, 'LHH — Fastballs');
  draw('scout-lhh-bb', 'Left',  r=>r.BBc, 'LHH — Breaking Balls');
  draw('scout-lhh-ch', 'Left',  isCH,     'LHH — Changeups');

  const isDamage = r => r.InPlay && (r.H===1 || r.HardHit===1);
  zoneDensity(document.getElementById('scout-dmg-rhh'), rows.filter(r=>r.BatterSide==='Right' && isDamage(r)), 'Damage vs RHH');
  zoneDensity(document.getElementById('scout-dmg-lhh'), rows.filter(r=>r.BatterSide==='Left' && isDamage(r)), 'Damage vs LHH');
}
...
function initSidebar(){
  ...
  function refreshPlayerOptions(){
    ...
    const noun = state.mode === 'pitching' ? 'Pitcher' : (state.mode === 'scout' ? 'Opposing Pitcher' : 'Hitter');
    ...
    if (state.mode === 'scout'){
      ...
      document.getElementById('playerCount').textContent = `${players.length} opposing pitcher(s) in range`;
    } else {
